...
double Sphere::getACoeficient(Ray &ray)
{
	...
	return ray.getXDir()*ray.getXDir()
			+ ray.getYDir()*ray.getYDir()
				+ ray.getZDir()*ray.getZDir();
}

double Sphere::getBCoeficient(Ray &ray)
{
	//b = 2[ (x2 - x1) (x1 - x3) + (y2 - y1) (y1 - y3) + (z2 - z1) (z1 - z3) ]
	return 2*(ray.getXDir()*(ray.getInitX() - cX)
				+ ray.getYDir()*(ray.getInitY() - cY)
					+ ray.getZDir()*(ray.getInitZ() - cZ));
...
double Sphere::getCCoeficient(Ray &ray)
{
	...
	return cX*cX + cY*cY + cZ*cZ
			+ ray.getInitX()*ray.getInitX()
				+ ray.getInitY()*ray.getInitY()
					+ ray.getInitZ()*ray.getInitZ()
						- 2*(ray.getInitX()*cX
							+ ray.getInitY()*cY
								+ ray.getInitZ()*cZ)
									- radius*radius;
...
double Sphere::getX() { return cX; }
double Sphere::getY() { return cY; }
double Sphere::getZ() { return cZ; }
